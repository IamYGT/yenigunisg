					...
					if(!empty($_POST['referans_baslik']) || !empty($_POST['referans_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET referans_baslik = ?, referans_seo = ?, referans_description = ?, referans_resim = ?, dil_id = ?, referans_durum = ?, referans_ust_id = ?");
								$insert->execute(array($referans_baslik[$key], Seo_Link_Cevir($referans_baslik[$key]), $referans_description[$key], $referans_resim, $key, $referans_durum, $LastID));
...
?>
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Referans Adresi</label>
																<input type="text" name="referans_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Referans Adresi" required="">
															</div>
														</div>
					...
					if(isset($submitControl)){
						if(!empty($_POST['referans_baslik']) || !empty($_POST['referans_aciklama'])) {
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  referans_baslik = ?, referans_seo = ?, referans_description = ?, referans_resim = ?, referans_durum = ? WHERE referans_ust_id = ? AND dil_id = ?");
									$update->execute(array($referans_baslik[$key], Seo_Link_Cevir($referans_baslik[$key]), $referans_description[$key], $referans_resim, $referans_durum, $id, $key));
								}
							}
							if ($update) {
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Referans Adresi</label>
																<input type="text" name="referans_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Referans Adresi" required="" value="<?php echo GetTableValue("referans_description",TABLE,"where referans_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
